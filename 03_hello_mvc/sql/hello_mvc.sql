...
-- 첨부파일 삭제
select * from attachment where board_no = 123;
...
-- 댓글 데이터 생성
select * from board order by no desc; -- 123
select * from board_comment where board_no = 123;
...
-- connect by a = prior b : 부모/자식행의 연결조건 작성. prior키워드 부모컬럼 앞에 위치.
