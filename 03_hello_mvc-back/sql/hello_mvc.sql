...
-- 댓글 데이터 생성
select * from board order by no desc; -- 161
select * from board_comment where board_no = 161;
insert into
    board_comment
values(
...
-- 계층형 쿼리
-- 행과 행 사이의 부모/자식 관계를 고려하여 조회
-- start with 조건절: 최상위 부모행의 조건을 작성
-- connect by a = prior b: 부모/자식 행의 연결조건 작성. prior 키워드 부모 컬럼 앞에 위치
-- level이라는 가상컬럼 제공. 최상위부모부터 1씩 증가처리
